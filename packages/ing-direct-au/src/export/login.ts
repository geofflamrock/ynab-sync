...
export async function login(
  ...
  for (const char of accessCode.split("")) {
    ...
    console.log(`Clicking pin number with index ${index}`);
  ...
  const loginButtonPromise = page.click("#login-btn");
  const authTokenRequest = page.waitForResponse(
    (r) =>
      r.url() ===
  ...
  try {
    await Promise.all([authTokenRequest, loginButtonPromise]);
  } catch (e) {
    page.screenshot({
      path: "./capture.png",
    });
  ...
  // const request = await authTokenRequest;
  // const i = 0;
  // while (i <= 10) {
  ...
  const authToken = authTokenRequest
    .then((res) => {
  ...
  return randomisedKeys.map((base64str, i) => {
    ...
    for (let j = 0; j < keypadImages.length; j++) {
      if (
        pixelmatch(
          img.data,
          keypadImages[j]?.data || Buffer.alloc(0), // We have to do this else typescript complains, even if a check is added for undefined
          null,
          180,
          110
        ) < 10
